...
!(function (exports) {
  ...
  simplexn.PointSet.prototype.prod = function(pointset) {
    ...
    var newPointset = new PointSet(newSize, newRn);
    var newPoint;
    var i = 0;

    this.forEach(function (point1) {
      pointset.forEach(function (point2) {
        newPoint = new Float32Array(newRn);
        newPoint.set(point1);
        newPoint.set(point2, rn);
        newPointset.set(newPoint, i++);
      });
    });

    this.points = newPointset;
    this.rn = newRn;
  ...
  simplexn.SimplicialComplex.prototype.__defineGetter__('maxCells', function () {
  ...
  simplexn.SimplicialComplex.prototype.prod = function(simpcomp) {
    ...
    var n = this.size;
    var pointset = simpcomp.pointset.clone().prod(this.pointset);
    var quotes = [];
