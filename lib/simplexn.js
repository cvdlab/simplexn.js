...
!(function (exports) {
  ...
  simplexn.Topology = function (complex, dim) {
    ...
    var dim = dim || complex[0].length - 1;
...
/**
  ...
  simplexn.SimplicialComplex.prototype.extrude = function (hlist) {
    var hlist = hlist || [];
    var hlistLength = hlist.length; // nHeights
    var positiveQuotes = hlist.filter(function (h) {return h >= 0}).length;  // nPosHeights
    ...
    var newPointsetSize = (hlistLength + 1) * pointsetSize;
    ...
    newPointset.set(oldEmbeddedPoints);
    ...
    for (h = 0; h < hlistLength; h += 1) {
      ...
      newPointset.set(newPoints, (h + 1) * pointsetSize);
      ...
      if (hlist[h] >= 0) {
        for (c = 0; c < complexSize; c += 1) {
          ...
          for (i = 0; i < cellLength; i++) {
            tempIndx = complex[c*cellLength+i] + h * pointsetSize;
            temp[i] = tempIndx;
            temp[i+cellLength] = tempIndx + pointsetSize;
          }
          
          // pick faces from temp, cellLength by cellLength
