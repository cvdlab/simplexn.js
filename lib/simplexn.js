...
!(function (exports) {
  ...
  simplexn._areEqual = function (a1, a2) {
    var a1Len = a1.length
      , a2Len = a2.length
      , res = a1Len === a2Len && a1.constructor === a2.constructor
      , i
      ;

    if (res) {
      for (i = 0; i < a1Len && res; i++) {
        res &= a1[i] === a2[i];
      }
    }
...
/**
  ...
  simplexn.SimplicialComplex.prototype.extrude = function (hlist) {
    // var hlist = hlist || [] // hlist must be made by positive numbers or by an alternation of positive and negative
    //   , nHeights = hlist.length
    //   , dim = this.dim
    //   , newDim = dim + 1
    //   , vertices = this.vertices
    //   , verticesLen = this.vertices.length
    //   , nVertices =  verticesLen / dim
    //   , topology = this.topology
    //   , maxFacesDim = topology.length
    //   , faces = this.faces
    //   , facesLen = faces.length
    //   , nFaces = facesLen / maxFacesDim
    //   , abs = Math.abs
    //   , newVertices = new Float32Array(nHeights*newDim*nVertices)
    //   , newFaces = new Uint32Array(nFaces*newDim)
    //   , tempDim = 2*maxFacesDim
    //   , temp = new Uint32Array(tempDim)
    //   , res, heigth
    //   , h, f, i, j
    //   , vIndx = verticesLen
    //   , fIndx = 0 
    ...
    var newFaces = new Uint32Array(nPosHeights*nFaces*newDim*(newDim+1)) // non tengo ancora conto delle h negative
